...
PYTHON_SCRIPT_DIR=${PEGASO_COLLT_DIR}
PYTHON_SCRIPT=${1}
SCRIPTNAME=${0%.*} ; SCRIPTNAME=${SCRIPTNAME##*/}
LOGFILE="${PEGASO_COLLT_DIR%/*}/logs/${SCRIPTNAME}_${1%.*}_"`date "+%Y-%m-%d_%H-%M-%S"`.log
LOGFILE_DET="${PEGASO_COLLT_DIR%/*}/logs/${SCRIPTNAME}_${1%.*}_"`date "+%Y-%m-%d_%H-%M-%S"`_pyoutput.log
